...
// 接收请求的参数
const resolvePost = (req) => new Promise((res) => {
  let chunk = ''
  req.on('data', (data) => {
    chunk += data
  })
...
const mergeFileChunk = async (filePath, fileName, size) => {
  ...
  const arr = chunkPaths.map((chunkPath, index) => pipeStream(
...
server.on('request', async (req, res) => {
  ...
  console.log(req.url)
  if (req.url === '/verify') {
    const createUploadedList = async (fileName) => (fse.existsSync(path.resolve(UPLOAD_DIR, fileName))
